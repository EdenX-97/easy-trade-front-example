...
import SelectMotorCard from "@/components/car/SelectMotorCard"
...
const carTypes = [
  {
    ...
    value: 'SECONDHANDMOTORAD',
    label: t('secondHandMotor')
  },
]
...
// 获取所有品牌
const allBrands = ref([])
axios.get('/example/motor/getAllBrands').then((res) => {
...
const getSeries = function(loading) {
  axios.get('/example/motor/getAllSeriesByBrand', {
    params: {
      brand: brand.value
    }
  }).then((res) => {
...
const getModels = function(loading) {
  axios.get('/example/motor/getModelsBySeries', {
    params: {
      series: series.value
    }
  }).then((res) => {
